...
//	cli();
    ...
    debug("Page fault (PF) occurred at %x. PF context: %s - %s - %s - %s - %s", err_pos, page_present_str, readwrite, user_mode_str, reserved_write, instruction_fetch_str);
    debug("PF extra info => Int. no: %d; CS: %x; EIP: %x; ESP: %x; EFLAGS = %b", regs->int_no, regs->cs, regs->eip, regs->esp, regs->eflags);
}
...
//	char *pg1 = (char *) 0x400000;				/* kernel page 1 (4MB) 0x400000*/
    ...
    cprintf("page dir= %x --", page_directory);
    cprintf("first table = %x --", first_page_table);


    __load_page_directory(*page_directory);	// physical address TODO ver esto!!
    __enable_paging();
}