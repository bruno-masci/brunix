...
class Runner():
    ...
    def run_qemu(self, *monitors, **kw):
        ...
        def run_qemu_kw(target_base="qemu", make_args=[], timeout=5):
        ...
        try:
            ...
            self.__react([self.qemu], timeout=5)
            self.qemu.on_output = []
            if self.gdb is None:
...
""")
    ...
    def __monitor_start(self, output):
        if b"\n" in output:
            try:
                self.gdb = GDBClient(self.qemu.get_gdb_port(), timeout=5)
                raise TerminateTest
            except socket.error:
                pass
        if not len(output):
...
def stop_breakpoint(addr):
    ...
    def setup_breakpoint(runner):
        if isinstance(addr, str):
            addrs = [int(sym[:8], 16) for sym in open("brunix.sym") #open("obj/kern/kernel.sym")
